...
- Robot control room & Chinese translation room
- Where in the system reasoning occurs
  - trial and error and agentic CoT LLMs: the curation algorithm that verifies the logic/reasoning (proving a number prime, and checking the potential factors by dividing by prime numbers 2, 3, 5, 7)
  - trial and error in a human: the curation/vetting/validation/testing algorithm in our heads (e.g. multiplying factors to reproduce target)
  - a neural network is a decision tree
- What are the conceptual objects being manipulated/modeled/predicted
  - LLM - words disconnected from real world objects, it's like being in a Robot Control Room and trying to predict the next pattern of dashboard lights
  - a baby is corrected by the real world if they eat something bitter, or try to bite something hard, or touch something hot or cold or lean over too far in a chair or car seat, dopamine delights them each time they are able to see their body do something and associate it with their thoughts and sensations
